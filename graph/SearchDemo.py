...
def find_friend (tx,phone):
    for record in tx.run("MATCH (a:TELEPHONE) WHERE a.tel = '"+phone+"' RETURN a.idcard AS idcard, a.name AS name, a.tel AS tel"):
...
# pageRank
def algoPageRank(tx):
    for record in tx.run("CALL algo.pageRank.stream('Page', 'LINKS', {iterations:20, dampingFactor:0.85}) YIELD nodeId, "
